...
public class MyWork_Async_Синхронизация_Общая extends Worker {
    ...
    private void МетодБиндингаОбщая() throws InterruptedException {
        ...
        getApplicationContext().bindService(intentГлавнаяСинхрониазция,serviceConnectionAsync ,Context.BIND_AUTO_CREATE);
    } catch (Exception e) {
        e.printStackTrace();
        Log.e(this.getClass().getName(), "Ошибка " + e + " Метод :" + Thread.currentThread().getStackTrace()[2].getMethodName() +
                " Линия  :" + Thread.currentThread().getStackTrace()[2].getLineNumber());
...
// TODO: 26.03.2023 ВЫКЛЮЧАЕМ БИНДИНГ СИНХОРНИЗАЦИИ
            if (serviceConnectionAsync!=null) {
                getApplicationContext().unbindService(serviceConnectionAsync);
            }
// TODO: 25.03.2023
            Log.d(this.getClass().getName(),"\n" + " class " + Thread.currentThread().getStackTrace()[2].getClassName() + "\n" +
                    " metod " + Thread.currentThread().getStackTrace()[2].getMethodName() + "\n" +
