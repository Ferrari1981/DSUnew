 ...
 @Inject
   private Class_Generation_SendBroadcastReceiver_And_Firebase_OneSignal  class_generation_sendBroadcastReceiver_and_firebase_oneSignallass ;
    ...
    private void МетодПодключениекСлубе() {
        ...
        context.    bindService(intentОбноразоваяСинхронизациия,serviceConnectionAsyns ,Context.BIND_AUTO_CREATE);
    } catch (Exception e) {
        e.printStackTrace();
        Log.e(this.getClass().getName(), "Ошибка " + e + " Метод :" + Thread.currentThread().getStackTrace()[2].getMethodName() +
                " Линия  :" + Thread.currentThread().getStackTrace()[2].getLineNumber());
 ...
 try{
     ...
     myDataОтветОдноразовойСлужбы = new Data.Builder()
           ...
           .putBoolean("Proccesing_MyWork_Async_Синхронизация_Одноразовая",true)
             .build();
     // TODO: 26.03.2023 ЗАКРЫВАЕМ БИНДИНГ СИНХРОНИАЗЦИИ
     if (serviceConnectionAsyns!=null) {
         context.unbindService(serviceConnectionAsyns);
     }
// TODO: 26.03.2023
     Log.d(this.getClass().getName(),"\n" + " class " + Thread.currentThread().getStackTrace()[2].getClassName() + "\n" +
             " metod " + Thread.currentThread().getStackTrace()[2].getMethodName() + "\n" +
